...
public class TxRigidBody : TxBody
{
    ...
    protected override void OnBeforeStep()
    {
        if (m_rigidBody)
        {
            ...
            if (TxNative.WorldObjectGetEnabled(worldID, objectID)) TxNative.WorldObjectSetActive(worldID, objectID, !GetComponent<Rigidbody>().IsSleeping());
        }
        else
        {
    ...
    {
        if (m_rigidBody && TxNative.WorldObjectGetActive(worldID, objectID))
        {
            Rigidbody rb = m_rigidBody;
